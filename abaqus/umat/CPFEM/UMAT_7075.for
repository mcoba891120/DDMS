...
C       PROPS(52)           -- L_ssd, mean distance between precipitates
C       PROPS(53)           -- phi, efficiency factor
C       PROPS(54)           -- k20, the proportion of the dislocations annihilating
C                              without helped by Orowan loops
C       PROPS(55)           -- k2p, the proportion of the dislocations annihilating
C                              with helped by Orowan loops
C
C======================================================================
...
CFIXA
           ...
           TERMexp = -1/SQRT(RHOS)*PROP(13)/PROP(12)
           TERMk2 = PROP(14)*exp(TERMexp)+PROP(15)*(1-exp(TERMexp))

           TERM1=PROP(2)*PROP(3)*PROP(4)/2
           TERM2=RHOS+RHOG
C-----     TERM3=PROP(5)*SQRT(RHOS)-PROP(6)*RHOS
           TERM3=PROP(5)*SQRT(RHOS)-TERMk2*RHOS
           TERM4=(1.-(RHOG/PROP(8))**PROP(11))/PROP(4)/PROP(7)
           TERM5=-(TERM3+TERM4)/2/SQRT(TERM2**3)
           TERM6=(PROP(5)/2/SQRT(RHOS)-PROP(6))/SQRT(TERM2)
           TEMP=PROP(4)*PROP(7)*PROP(8)*SQRT(TERM2)
           TERM7=PROP(11)*(RHOG/PROP(8))**(PROP(11)-1)/TEMP

           DHSELF=((TERM5+TERM6)*TERM3+(TERM5-TERM7)*TERM4)*TERM1        
...
C-----     TERM3=PROP(5)*SQRT(RHOS)-PROP(6)*RHOS
           ...
           DHLATN=((TERM5+TERM6)*TERM3+(TERM5-TERM7)*TERM4)*TERM1*Q
...
C-----     Microstructure-based modelling of isotropic and kinematic
C-----     strain hardening in a precipitation-hardened aluminium alloy

           TERMexp = -1/SQRT(RHOS)*PROP(13)/PROP(12)
           TERMk2 = PROP(14)*exp(TERMexp)+PROP(15)*(1-exp(TERMexp))

           TERM1=PROP(2)*PROP(3)*PROP(4)/SQRT(RHOS+RHOG)
C-----     TERM2=PROP(5)*SQRT(RHOS)-PROP(6)*RHOS
           TERM2=PROP(5)*SQRT(RHOS)-TERMk2*RHOS
           TERM3=(1.-(RHOG/PROP(8))**PROP(11))/PROP(4)/PROP(7)
