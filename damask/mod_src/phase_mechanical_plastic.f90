...
submodule(phase:mechanical) plastic
  ...
  interface
    ...
    module subroutine phenopowerlaw_dotState(Mp,ph,en,Cauchy)
      ...
      ! real(pReal), dimension(12),   intent(in) :: &
      !   Cauchy
      integer,                      intent(in) :: &
        ph, &
        en
...
module subroutine plastic_init
  ...
  print'(/,a)', ' <<<+-  phase:mechanical:plastic_MOD init  -+>>>'
...
module function plastic_dotState(subdt,co,ip,el,ph,en) result(broken)
  ...
  real(pReal),              dimension(3,3) :: &
    Mp, Cauchy, Kirchhoff
  real(pReal),              dimension(3,3,12) :: &
    SchmidMatrixCur
  real(pReal),              dimension(12) :: &
    tau_slip_cur
  logical :: broken
  integer :: i
  ...
  Cauchy = 1.0_pReal/math_det33(phase_mechanical_Fe(ph)%data(1:3,1:3,en))* &
            matmul(matmul(phase_mechanical_Fe(ph)%data(1:3,1:3,en),phase_mechanical_S(ph)%data(1:3,1:3,en)), &
                  transpose(phase_mechanical_Fe(ph)%data(1:3,1:3,en)))

  Kirchhoff = matmul(phase_mechanical_P(ph)%data(1:3,1:3,en), transpose(phase_mechanical_F(ph)%data(1:3,1:3,en)))

  SchmidMatrixCur = lattice_SchmidMatrix_slip_currentConfig([12],"cF",0.0_pReal,phase_mechanical_F(ph)%data(1:3,1:3,en))
  do i = 1,12
    tau_slip_cur(i) = math_tensordot(Kirchhoff, SchmidMatrixCur)
  end do

  ! if (en==1) then 
  !   print *, '-----'
  !   print *, SchmidMatrixCur(:,:,1:3)
  !   print *, '-----'
  ! end if 
  ...
  plasticType: select case (phase_plasticity(ph))
    ...
    case (PLASTICITY_PHENOPOWERLAW_ID) plasticType
      call phenopowerlaw_dotState(Mp,ph,en,Cauchy)
...
!--------------------------------------------------------------------------------------------------
module function plastic_deltaState(ph, en, Delta_t) result(broken)

  integer, intent(in) :: &
    ph, &
    en
  real(pReal), intent(in) :: Delta_t  !!! region start/end
  logical :: &
    broken
  ...
  plasticType: select case (phase_plasticity(ph))
    ...
    !!! region start
    case (PLASTICITY_PHENOPOWERLAW_ID) plasticType
      call plastic_phenopowerlaw_deltaState(Mp,ph,en,Delta_t,Cauchy)
      broken = any(IEEE_is_NaN(plasticState(ph)%deltaState(:,en)))      
    !!! region end
  ...
  if(.not. broken) then
    select case(phase_plasticity(ph))
      case (PLASTICITY_NONLOCAL_ID,PLASTICITY_KINEHARDENING_ID,PLASTICITY_PHENOPOWERLAW_ID) ! region start/end
